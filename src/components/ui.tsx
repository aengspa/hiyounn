...
// ── 문서 어휘 라벨 (Phase A) ──
import type { TestStatus, ExecutionTier } from "@/lib/domain/types";
...
export const TEST_STATUS_LABEL: Record<TestStatus, string> = {
  ...
  NOT_TESTED: "미검사",
  TEST_FAILED: "검사 실패",
  FIXED_VERIFIED: "수정 검증됨",
  REGRESSION_FAILED: "기능 회귀 실패",
};

const TEST_STATUS_STYLE: Record<TestStatus, string> = {
  CONFIRMED: "bg-amber-100 text-amber-800",
  SUSPECTED: "bg-slate-100 text-slate-700",
  ...
  FIXED_VERIFIED: "bg-emerald-100 text-emerald-800",
  REGRESSION_FAILED: "bg-red-100 text-red-800",
};
...
export function TestStatusBadge({ status }: { status: TestStatus }) {
  return (
    <span
      className={`inline-flex items-center rounded px-1.5 py-0.5 text-[11px] font-medium ${TEST_STATUS_STYLE[status]}`}
    >
...
export const TIER_LABEL: Record<ExecutionTier, string> = {
  PASSIVE: "정적 분석",
  SAFE_ACTIVE: "비파괴 능동 검사",
  ISOLATED_ACTIVE: "격리 공격 재현",
  PATCH: "패치 생성",
